...
Disusun untuk memenuhi sebagian persyaratan untuk memperoleh gelar Sarjana Komputer  
Jurusan Informatika
...
- BAB II TINJAUAN PUSTAKA (7)
  ...
  - 2.6 Arsitektur Sistem Laravel-FastAPI dan Docker (12)
  - 2.7 Penerapan Machine Learning dalam Prediksi Konsumsi Pangan (13)
  - 2.8 Implementasi LSTM untuk Time Series Forecasting (14)
  - 2.9 Penelitian Terkait Prediksi Konsumsi Pangan di Indonesia (14)
  - 2.10 Gap Analysis (15)
  - 2.11 Kerangka Konseptual (15)
- BAB III METODOLOGI (17)
  - 3.1 Pendekatan dan Jenis Penelitian (17)
  - 3.2 Kerangka Kerja CRISP-DM (19)
...
```
(1 − φ₁L − φ₂L² − ... − φₚLᵖ)(1 − L)ᵈXₜ = (1 + θ₁L + θ₂L² + ... + θqLq)εₜ  ... (4)
```
...
di mana Sₜ adalah smoothed value pada waktu t, α adalah smoothing parameter (0 < α < 1), dan Χₜ adalah actual value pada waktu t.
...
di mana fₜ adalah forget gate output pada waktu t, σ adalah sigmoid function, Wf adalah weight matrix untuk forget gate, hₜ₋₁ adalah hidden state sebelumnya, 𝓍ₜ adalah input pada waktu t, dan bf adalah bias vector untuk forget gate.
...
```
iₜ = σ(Wi · [hₜ₋₁, 𝓍ₜ] + bi)  ... (14)
C̅ₜ = tanh(WC · [hₜ₋₁, 𝓍ₜ] + bC)  ... (15)
```

di mana iₜ adalah input gate output, C̅ₜ adalah kandidat nilai cell state baru, Wi, WC adalah weight matrices, dan bi, bC adalah bias vectors.
...
```
ŷ_ensemble = Σ(i=1 to n) wᵢ · ŷᵢ  ... (20)
```

dengan constraint Σ(i=1 to n) wᵢ = 1 dan wᵢ ≥ 0, di mana ŷ_ensemble adalah prediksi ensemble, wᵢ adalah weight untuk model ke-i, ŷᵢ adalah prediksi dari model ke-i, dan n adalah jumlah model dalam ensemble.
...
```
MAPE = (100%/n) Σ(i=1 to n)|yᵢ − ŷᵢ / yᵢ|  ... (27)
```
...
Laravel menyediakan robust foundation untuk web application development dengan features seperti Eloquent ORM, Livewire reactive components, dan Blade templating engine yang memudahkan development of interactive dashboard dan real-time user interactions.
...
![Gambar 1. Tahapan Research and Development](diagram)
...
![Gambar 2. Diagram Alur CRISP-DM](diagram)
...
```
MAPE = (100%/n) Σ(i=1 to n)|yᵢ − ŷᵢ / yᵢ|  ... (28)
```
...
```
r = Σ(i=1 to n)(xᵢ − x̄)(yᵢ − ȳ) / √[Σ(i=1 to n)(xᵢ − x̄)² Σ(i=1 to n)(yᵢ − ȳ)²]  ... (31)
```
...
```
ŷ_ensemble = Σ(i=1 to n) wᵢ · ŷᵢ  ... (44)
```

dengan constraint Σ(i=1 to n) wᵢ = 1 dan wᵢ ≥ 0.
...
Asian Development Bank. (2023). *Asian Development Outlook April 2023*. Asian Development Bank (Issue April).
