<x-layouts.pemerintah title="Data Lahan Pertanian - Panel Pemerintah - {{ config('app.name') }}">
    <div class="container mx-auto px-4 py-8" x-data="lahanData()">
        {{-- Header --}}
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-neutral-900 dark:text-white">Data Lahan Pertanian</h1>
            <p class="text-neutral-600 dark:text-neutral-400 mt-1">Informasi Luas dan Produktivitas Lahan Pertanian per Wilayah</p>
        </div>
        ...
        <div x-show="!loading && data.length === 0 && !hasSearched" 
            ...
            <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Pilih filter dan klik "Tampilkan Data" untuk melihat data lahan.</p>
        </div>
    </div>
        ...
        function lahanData() {
            return {
                ...
                async loadVariabels() {
                    ...
                    try {
                        const response = await fetch(`/pemerintah/lahan/variabels/${this.filters.topik}`);
                ...
                async filterData() {
                    ...
                    try {
                        ...
                        const response = await fetch('/pemerintah/lahan/filter', {
                            method: 'POST',
                            body: formData,
                            headers: {
                ...
                exportData() {
                    ...
                    form.method = 'POST';
                    form.action = '/pemerintah/lahan/export';
