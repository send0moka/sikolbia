<x-layouts.pemerintah title="Data Benih & Pupuk - Panel Pemerintah - {{ config('app.name') }}">
    <div class="container mx-auto px-4 py-8" x-data="benihPupukData()">
        {{-- Header --}}
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-neutral-900 dark:text-white">Data Benih & Pupuk</h1>
            <p class="text-neutral-600 dark:text-neutral-400 mt-1">Informasi Distribusi Benih dan Pupuk per Wilayah</p>
        </div>
        ...
        <div x-show="!loading && data.length === 0 && !hasSearched" 
            ...
            <p class="mt-1 text-sm text-neutral-500 dark:text-neutral-400">Pilih filter dan klik "Tampilkan Data" untuk melihat data benih & pupuk.</p>
        </div>
    </div>

    @push('scripts')
    <script>
        function benihPupukData() {
            return {
                filters: {
                    topik: '',
                    variabel: '',
                ...
                async loadVariabels() {
                    ...
                    try {
                        const response = await fetch(`/pemerintah/benih-pupuk/variabels/${this.filters.topik}`);
                ...
                async filterData() {
                    ...
                    try {
                        ...
                        const response = await fetch('/pemerintah/benih-pupuk/filter', {
                            method: 'POST',
                            body: formData,
                            headers: {
                ...
                exportData() {
                    ...
                    form.method = 'POST';
                    form.action = '/pemerintah/benih-pupuk/export';
