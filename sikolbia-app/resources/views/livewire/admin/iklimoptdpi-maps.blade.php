<div>
    ...
    <div class="mb-6">
        <div class="flex justify-between items-center">
            <div>
                <h1 class="text-2xl font-semibold text-neutral-900 dark:text-white">Peta Iklim Opt DPI</h1>
                <p class="mt-1 text-sm text-neutral-600 dark:text-neutral-400">
                    Visualisasi data iklim optimalisasi DPI berdasarkan wilayah
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="mb-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div>
            <label class="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">Topik Iklim</label>
    ...
    <div class="bg-white dark:bg-neutral-800 rounded-lg border border-neutral-200 dark:border-neutral-700 p-6 mb-6">
        ...
        <div id="iklimoptdpi-map" class="h-96 rounded-lg border border-neutral-200 dark:border-neutral-600"></div>
    </div>

    @script
    <script>
        let iklimoptdpiMap = null;

        function initializeIklimoptdpiMap() {
            // Check if Leaflet is loaded
            if (typeof L === 'undefined') {
                setTimeout(initializeIklimoptdpiMap, 100);
                return;
            }

            // Check if map container exists
            const mapContainer = document.getElementById('iklimoptdpi-map');
            if (!mapContainer) {
                setTimeout(initializeIklimoptdpiMap, 100);
                return;
            }
            ...
            if (iklimoptdpiMap) {
                iklimoptdpiMap.remove();
            }

            iklimoptdpiMap = L.map('iklimoptdpi-map').setView([-2.5489, 118.0149], 5);

            L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
                attribution: '© OpenStreetMap contributors',
                maxZoom: 18
            }).addTo(iklimoptdpiMap);
            ...
            if (mapData.length > 0) {
                const group = new L.featureGroup(iklimoptdpiMap._layers);
                if (Object.keys(group._layers).length > 0) {
                    iklimoptdpiMap.fitBounds(group.getBounds().pad(0.1));
                }
            }
        }

        // Initialize map when Livewire component is ready
        document.addEventListener('livewire:navigated', function() {
            setTimeout(initializeIklimoptdpiMap, 300);
        });
        ...
        // Initialize after Livewire component is hydrated
        Livewire.hook('morph.updated', () => {
            setTimeout(initializeIklimoptdpiMap, 200);
        });
