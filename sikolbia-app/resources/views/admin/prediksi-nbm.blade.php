<x-layouts.app title="Prediksi NBM - Panel Admin - {{ config('app.name') }}">
    <!-- Chart.js CDN -->
    ...
    <div class="container mx-auto px-4 py-8">
                ...
                <div class="bg-white dark:bg-neutral-800 rounded-lg shadow p-6 h-full">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <div id="exportButtons" style="display: none;" class="flex gap-2">
                            ...
                            <a href="{{ route('admin.prediksi-nbm.history') }}" class="px-3 py-1.5 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition flex items-center gap-2">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <!-- Chart Visualization Section (same as pemerintah) -->
        ...
        <!-- AI Insights Section (same structure as pemerintah) -->
        ...
        <!-- Historical Data Reference (same as pemerintah) -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Load komoditi when kelompok changes - using pemerintah API
            kelompokSelect.addEventListener('change', function() {
                ...
                // Use pemerintah API for komoditi lookup (same data source)
            ...
            // Handle prediksi button - use admin route
            prediksiBtn.addEventListener('click', function() {
                ...
                // Use admin route
                fetch('{{ route("admin.prediksi-nbm.run") }}', {
                    method: 'POST',
                    headers: {
            ...
            // Display results function (same logic as pemerintah, but with admin routes)
            function displayResults(data) {
                ...
                if (data.warning || (data.has_data === false)) {
                    ...
                    html += `<div class="flex"><div class="flex-shrink-0">`;
                    ...
                    html += `</svg></div><div class="ml-3">`;
                    html += `<p class="text-sm text-yellow-700 dark:text-yellow-200"><strong>Perhatian:</strong> ${data.warning || 'Data historis untuk komoditi ini kosong atau tidak tersedia.'}</p>`;
                    html += `</div></div></div>`;
                }
                ...
                if (data.prediction && data.prediction.length > 0) {
                    ...
                    if (data.uncertainty_metrics) {
                        ...
                        html += `<p class="text-xs text-blue-800 dark:text-blue-200"><strong>Metrik Ketidakpastian:</strong> Interval: ${data.uncertainty_metrics.interval_width.toFixed(2)}, `;
                        html += `Relatif: ${data.uncertainty_metrics.relative_uncertainty.toFixed(2)}%, `;
                        html += `Confidence: ${(data.uncertainty_metrics.confidence_level * 100).toFixed(0)}%</p></div>`;
                    }
                } else {
                ...
                if (data.model_info) {
                    ...
                    if (versionElement && data.model_info.model_version) {
                        versionElement.textContent = data.model_info.model_version;
                    }
                }
                ...
                if (tbody) {
                    if (data.historical && data.historical.length > 0) {
                        ...
                        tbody.innerHTML = `<tr class="border-b border-neutral-200 dark:border-neutral-700">
                            <td colspan="5" class="px-6 py-8 text-center text-neutral-500 dark:text-neutral-400">
            ...
            // Fetch AI insights using admin route
            async function fetchInsights(data) {
                ...
                try {
                    const response = await fetch('{{ route("admin.prediksi-nbm.insights") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                    ...
                    if (result.success) {
                        ...
                        insightsContent.innerHTML = `<div class="bg-red-50 dark:bg-red-900/20 border-l-4 border-red-400 p-4">
            ...
            // Display insights (reusing same logic)
            function displayInsights(insights) {
                ...
                html += `<div class="${trendCardClass} p-4 rounded"><div class="flex items-start gap-3">
                    <div class="text-3xl">${trend.icon}</div><div class="flex-1">
                ...
                html += `<div class="${riskCardClass} p-4 rounded"><div class="flex items-start gap-3">
                    <div class="text-3xl">⚠️</div><div class="flex-1">
                ...
                html += `<div class="bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 p-4 rounded">
                ...
                if (anomaly.has_anomalies) {
                    html += `<div class="bg-yellow-50 dark:bg-yellow-900/20 border-l-4 border-yellow-400 p-4 rounded mt-4">
                        <h3 class="font-semibold text-yellow-900 dark:text-yellow-100 mb-2 flex items-center gap-2"><span>⚠️</span> Anomali Terdeteksi</h3>
                ...
                html += `<div class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 p-4 rounded mt-4">
                ...
                html += `<div class="bg-neutral-50 dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-700 p-4 rounded mt-4">
            ...
            // Save prediction handler with admin route
            document.getElementById('saveBtn')?.addEventListener('click', async function() {
                ...
                try {
                    const response = await fetch('{{ route("admin.prediksi-nbm.save") }}', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json',
                            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content
            ...
            // Initialize export handlers with admin routes
            if (typeof window.setupExportHandlers === 'function') {
                window.setupExportHandlers(
                    '{{ route("admin.prediksi-nbm.export-excel") }}',
                    '{{ route("admin.prediksi-nbm.export-pdf") }}'
                );
            }
        });
    </script>
